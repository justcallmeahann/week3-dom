...
function validationSignup(event) {
  ...
  if(email.value == ""){
    return Notification("error", "Please input the email");
  } else if(password.value == ""){
    return Notification("error", "Please input the password");
  } else if(phoneNumber.value == ""){
    return Notification("error", "Please input the phone number");
  } else if (!email.value.match(emailRegex)) {
    return Notification("error", "Please enter a valid email.");
  } else if (!phoneNumber.value.match(phoneRegex)) {
    return Notification("error", "Please enter a valid phone number.");
  } else if(email.value === regEmail){
    return Notification("error", "Email has already been taken");
  }
